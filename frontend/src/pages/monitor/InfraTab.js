...
function DetailPanel({ selected, statuses, activity, onClose, onNavigate, onNavigateToTab }) {
  if (!selected) return null;
  const s = statuses[selected.id];
  const st = s?.status || 'UNKNOWN';
  const entityInfo = SERVICE_ENTITIES[selected.id];
  const dbInfo = DB_TABLES[selected.id];
  const isDb = selected.type === 'db';
  ...
  return (
    <div className="monitor-detail show">
      ...
      {isService && entityInfo && (
        <div className="detail-entities">
          <div className="detail-actions-title">Entidades ({entityInfo.entities.length}):</div>
          {entityInfo.entities.map(e => (
            <div key={e.name} className="entity-item">
              <span className="entity-name">{e.name}</span>
              <span className="entity-table">{e.table}</span>
              <span className="entity-fields">{e.fields.length} campos</span>
            </div>
          ))}
          {onNavigateToTab && (
            <button className="detail-navigate" onClick={() => onNavigateToTab('estructura', selected.id)}>
      ...
      {isDb && dbInfo && (
        <div className="detail-entities">
          <div className="detail-actions-title">Tablas ({dbInfo.tables.length}):</div>
          {dbInfo.tables.map(t => (
            <div key={t.name} className="entity-item">
              <span className="entity-name">{t.entity}</span>
              <span className="entity-table">{t.name}</span>
              <span className="entity-fields">{t.count} registros</span>
            </div>
          ))}
          {onNavigateToTab && (
            <button className="detail-navigate" onClick={() => onNavigateToTab('datos', selected.id)}>
              Ver datos →
            </button>
          )}
        </div>
...
export default function InfraTab({ onNavigateToTab }) {
